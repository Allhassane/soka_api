    ...
    <div class="main-content">
        <section class="section">
            <ul class="breadcrumb breadcrumb-style ">
                ...
                <li class="breadcrumb-item active">Edition du contenu des axes stratégiques</li>
            </ul>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>Modifier le contenu {{$data->libelle}}</h4>
                            </div>
                            <div class="card-body">
                                <form class="user" method="post" action="{{route('axe_item.update',$data->id)}}">
                                    @csrf
                                    <div class="form-group row">
                                        <div class="col-12">
                                            <label for="">Choisir le nom de l'axe</label>
                                            <select name="domaine_id" class="form-control form-control-user" id="exampleFirstName"
                                                   placeholder="" required>
                                                   @foreach ($axes as $axe)
                                                     <option value="{{ $axe->id }}" @if($data->domaine_id==$axe->id) selected @endif>{{ $axe->title }}</option>
                                                   @endforeach
                                                   </select>
                                                   <br>
                                        </div>
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="libelle" value="{{ $data->libelle }}" class="form-control form-control-user" id="exampleFirstName"
