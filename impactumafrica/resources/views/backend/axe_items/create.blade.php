    ...
    <div class="main-content">
        <section class="section">
            <ul class="breadcrumb breadcrumb-style ">
                ...
                <li class="breadcrumb-item active">Enregistrement du contenu des sous axes strategiques</li>
            </ul>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>Enregistrement du contenu des sous des axes strategiques</h4>
                            </div>
                            <div class="card-body">
                                <form class="user" method="post" action="{{route('axe_item.store')}}">
                                    @csrf
                                    <div class="form-group row">
                                        <div class="col-12">
                                            <label for="">Saisir le nom du domaine d'activité</label>
                                            <select name="domaine_id" class="form-control form-control-user" id="exampleFirstName"
                                                   placeholder="" required>
                                                   @foreach ($axes as $axe)
                                                     <option value="{{ $axe->id }}">{{ $axe->title }}</option>
                                                   @endforeach
                                                   </select>
                                                   <br>
                                        </div>
                                        <div class="col-12">
                                            <label for="">Saisir le sous axe</label>
                                            <input type="text" name="libelle" value="{{ old('libelle') }}" class="form-control form-control-user" id="exampleFirstName"
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-12">
                                            ...
                                            <textarea id="description" name="description" class="form-control"
                                                      rows="5" cols="" placeholder="Saisir la description" required>

                                            </textarea>
                                        </div>
                                    </div>
                                    <center>
                                        <button type="submit" class="btn btn-primary btn-user ">Enregistrer</button>
                                        <a href="{{route('axe_item.list')}}" class="btn btn-danger">Annuler</a>
                                                       </center>
                                </form>
                            </div>
                        </div>
