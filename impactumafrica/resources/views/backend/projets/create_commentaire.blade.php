    ...
    <div class="main-content">
        <section class="section">
            <ul class="breadcrumb breadcrumb-style ">
                ...
                <li class="breadcrumb-item active">Enregistrement des resumé du projet {{$data->title}}</li>
            </ul>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form class="user" method="post" action="{{route('projet.store_commentaire',$data->id)}}" enctype="multipart/form-data">
                                    @csrf
                                    <h5 class="text-danger">Information du projet</h5>
                                    <input type="hidden" name="id" value="{{$data->id}}">
                                    <div class="form-group">
                                        <div class="col-sm-12">
                                            <label for="">Saisir le résumé</label>
                                            <textarea id="description" name="commentaire" class="form-control"
                                                      rows="6" cols="" placeholder="Saisir le commentaire" required>
                                                {!! $data->commentaire !!}
                                        </textarea>
                                        </div>
                                    </div>
                                    <center>
                                        <button type="submit" class="btn btn-primary btn-user ">Enregistrer le commentaire ou resumé</button>
                                        <a href="{{route('projet.list')}}" class="btn btn-danger">Annuler</a>
                                    </center>
                                </form>
                            </div>
                        </div>
