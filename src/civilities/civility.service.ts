...
export class CivilityService {
  ...
  async findAll(admin_uuid: string) {
    const division = await this.civilitiesRepo.find({
      order: { name: 'DESC' },
    });
  ...
  async store(payload: any,admin_uuid) {
    ...
    const newDivision = this.civilitiesRepo.create({
      name: payload.name,
      sigle: payload.sigle,
    ...
    const saved = await this.civilitiesRepo.save(newDivision);

    return saved;
  }

  async findOne(uuid: string,admin_uuid) {
    const division = await this.civilitiesRepo.findOne({ where: { uuid } });
  ...
  async delete(uuid: string,admin_uuid:string) {
    const civilite = await this.civilitiesRepo.findOne({ where: { uuid } });
    ...
    await this.logService.logAction(
      'civilities-delete',
      admin.id,
      "Suppression de la civilité "+civilite.name+" pour uuid"+civilite.uuid,
    );

   return await this.civilitiesRepo.softRemove(civilite);
