...
@Entity({ name: 'cities' })
export class CityEntity extends DateTimeEntity {
  ...
  // La colonne réelle `cities.uuid` a un DEFAULT NULL (synchronize OFF), donc le
  // `default: () => '(UUID())'` ci-dessus n'est jamais appliqué → les localités créées
