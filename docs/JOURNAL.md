...
- **🐛 Fait — diagnostic.** `ImportService.buildPayload()` recopiait la cellule Excel « Matricule »
  **verbatim**, via un `put()` qui **ignore les valeurs vides** ; le chemin `create`
  (`import.service.ts`, `commit()`) construit l'entité et la sauve **sans passer par
  `MemberService.store()`**, seul détenteur de la génération. Cellule vide ⇒ membre sans matricule ;
  cellule contenant `sss` ⇒ matricule `sss`. Corrélation : les 235 partagent l'`admin_uuid` des
  79 lots d'import, dans leur fenêtre exacte. Sur les **271** membres créés par l'import :
  **235 sans matricule, 32 avec le contenu brut du tableur, 4 corrects**.
- **Décision — la règle remonte dans un service partagé.** Nouveau `MatriculeService`
  (`src/members/matricule.service.ts`) + `MatriculeModule`, consommé par `MemberService.store()`
  **et** par l'import. **Même famille que `MemberAccountService`** (360 membres sans compte,
  corrigé le 2026-08-01, dans cette même fonction) : c'est la deuxième fois que l'import diverge de
  `store()` en silence. Le module est volontairement minuscule (dépôt `members` seul) — tirer
  `MembersModule` dans `ImportModule` créerait un cycle avec ses `forwardRef`.
- **Fait — le commentaire de `store()` affirmait le contraire du code** (« L'import de masse, lui,
  l'appelle toujours en direct ») : il laissait croire que l'import héritait de la génération.
  Corrigé — c'est précisément ce genre de note qui a rendu le défaut invisible.
- **Fait — `buildPayload` filtre par `isPlausible()`.** Sont acceptés le canonique `AA-NNNN` et la
  numérotation héritée tout-chiffres (`0007283`, **24 fiches — de vrais identifiants**) ; refusés
  le texte libre et les nombres courts, qui sont en pratique des **numéros de ligne** de tableur.
- **Fait — débordement à 9999 traité.** `padStart(4)` est désormais un **minimum**, pas une largeur
  fixe : `26-10000` est valide. `MAX(id)` valait 8 270 — le cas était à ~1 700 créations. Un test
  le verrouille pour que personne ne « corrige » l'élargissement en tronquant.
- **Fait — rattrapage `seed:fix-missing-matricule`** (le script était déclaré dans `package.json`
  sans que le fichier existe). `AA-NNNN` = année de création + `id` de la fiche : c'est la
  sémantique même du générateur (`MAX(id)+1` = l'`id` que la ligne va recevoir), et l'`id` d'une
  fiche existante ne sera jamais réémis. **0 collision, 0 doublon interne** (vérifié avant
  écriture). Joué en `--dry-run` puis `--confirm` sur instantané : **235 écrits, 0 restant**.
- **Décision — ne PAS toucher aux matricules hors format**, sauf ceux qui bloquent l'unicité. Les
  24 hérités sont de vrais identifiants ; les 18 numéros de ligne et 3 textes libres uniques
  relèvent d'un arbitrage métier. Seul `--liberer-doublons` régénère les **10 fiches**
  (ids 7987-7996) portant une note de saisie en double — le seul cas qui empêchait
  `UQ_members_matricule` d'exister. Ancienne valeur conservée dans le rapport Excel.
- **Fait — migration `AddMembersMatriculeUniqueIndex`** (`UQ_members_matricule`), que
  `CreateMemberRegistration` avait volontairement sautée. ⚠️ Elle **refuse de s'appliquer** si des
  doublons subsistent, en nommant la commande de rattrapage. Les deux alternatives étaient pires :
  un index simple porterait un nom `UQ_` mensonger et différerait d'un environnement à l'autre ;
  dédoublonner dans la migration modifierait des données métier au démarrage de l'API, sans
  relecture ni rapport. NULL reste permis (MySQL l'autorise sous un UNIQUE).
- **Fait — 16 tests** : `matricule.service.spec.ts` (7), `import-matricule.spec.ts` (5, sur la
  cause racine : le filtrage de la cellule du tableur) et
  `add-members-matricule-unique-index.spec.ts` (4, dont le **refus** — le comportement qui compte
  en prod et qu'on ne peut pas observer sur une base déjà rattrapée).
- **Fait — `.gitignore`** : les rapports Excel des seeds tombent à la racine du repo et portent des
  **noms de membres**. Ils n'étaient couverts par aucune règle (`seed:create-missing-user-accounts`
  était dans le même cas depuis le 2026-08-01).
- **Vérifié :** `tsc` vert, suite complète à **310 passés** (les 4 échecs `location/*` et
  `module/*` sont préexistants et sans rapport : modules Nest de test montés sans repository).
  API redémarrée sans erreur de résolution DI. Index confirmé `Non_unique = 0`.
- **⚠️ TODO — la production n'est PAS rattrapée.** Y jouer le seed **avant** la migration, sinon
  le déploiement s'arrête sur le refus (message explicite). Ordre : `-- --dry-run`, relecture de
  l'Excel, `-- --liberer-doublons --confirm`, puis `migration:run`.
- **TODO :** arbitrer avec l'équipe le sort des 21 matricules non conformes mais uniques
  (18 numéros de ligne, `sss`, `XXXXX`, `22:335-0218`) — listés dans le rapport du seed.
- **🐛 Signalé, non corrigé — `npm run lint` est cassé sous Windows, pour tout le monde.**
  `eslint.config.mjs:19` fait `tsconfigRootDir: new URL('.', import.meta.url).pathname`, qui rend
  `/C:/laragon/...` — ESLint refuse (« must be an absolute path »). **Toutes** les analyses
  échouent en « Parsing error », y compris sur des fichiers jamais modifiés (vérifié sur
  `src/app.module.ts`), et sous PowerShell comme sous Git Bash : ce n'est pas un effet du shell.
  Correctif = une ligne (`fileURLToPath(...)` au lieu de `.pathname`), **non appliqué** :
  `eslint.config.mjs` est un fichier partagé, à annoncer avant modification. En attendant, lancer
  `npx eslint` ne remplace pas `tsc` — c'est `tsc --noEmit` + les tests qui font foi ici.
  ⚠️ Ne pas lancer `npm run lint` à l'aveugle une fois réparé : le script porte `--fix` sur tout
  `src`, il reformaterait le code des autres modules et polluerait le commit.

## 2026-09-08 — Cascade de destination d'un transfert : la lecture refusait ce que l'écriture accepte — module `membres`
**Contexte :** remontée terrain — dans les transferts, les menus déroulants de structures restent
vides **dès le palier « Centre régional »**. Reproduit uniquement pour les non-administrateurs :
côté admin l'écran fonctionne, ce qui a longtemps fait passer le défaut pour un aléa d'affichage.
- **🐛 Fait — diagnostic.** `StructureCascadeSelect` alimentait ses cinq menus par
  `GET /structure/childrens/:uuid`, gardée par `assertNavigable()`
  (`structure.controller.ts`) → `assertStructureNavigable()`
  (`structure-tree.service.ts:1903`). Cette barrière n'autorise que **le sous-arbre de l'appelant
  et sa chaîne d'ancêtres**, et sort d'emblée si `isAdmin === true` — d'où l'asymétrie constatée.
  Mesure en base pour un responsable de district réel (ANOUANZE) : **1 région navigable sur 4,
  1 centre régional sur 17, 1 centre sur 54, 1 chapitre sur 129**. Le premier menu, lui, est servi
  par `/structure/childrens/_` **sans `uuid`** : `assertNavigable` n'a rien à contrôler et renvoie
  les 4 régions à tout le monde — le mur tombe donc exactement au palier suivant, « Centre
  régional », ce que décrivait la remontée.
- **Décision — c'était une incohérence lecture/écriture, pas un durcissement volontaire.**
  `MemberTransferService.create()` ne vérifie le périmètre que sur le district **source** (R2) ;
  le district **cible** est délibérément libre, sa validation revenant à l'approbateur (R3).
  L'API acceptait donc en écriture une cible que la route de lecture refusait d'afficher.
  `assertNavigable` avait été conçue pour le **filtre du tableau de bord**, où l'on ne descend que
  chez soi ; la réutiliser pour désigner une structure d'**accueil** était le contresens.
- **Fait — route dédiée `GET /structure/transfer-targets`**, sans contrôle de périmètre, sous
  `@RequirePermissions('membres_initier_transfert')` (déjà à `status=1` pour `RESPONSABLE` :
  **aucune migration**), bornée au **district** — descendre plus bas lèverait un `400`, groupes et
  sous-groupes relevant de l'approbateur. `/structure/childrens` reste **inchangée** avec sa
  barrière : le dashboard, `MembreTable` et `member-filters` doivent, eux, rester bornés.
- **Décision — route dédiée plutôt qu'assouplissement d'`assertNavigable`.** Toucher à la barrière
  aurait ouvert d'un coup les trois écrans de filtrage, pour un besoin qui ne concerne qu'un seul
  cas d'usage. Ce qu'on expose ici reste étroit : des **noms de structures jusqu'au district**,
  pour les seuls porteurs du droit d'initier un transfert. Aucune donnée de membre, aucun effectif.
- **⚠️ Fait — déclarée AVANT `@Get(':uuid')`.** Nest résout dans l'ordre de déclaration : un
  segment unique placé après aurait été capturé par le paramètre. Vérifié au log
  `RouterExplorer` (`transfer-targets` mappée juste avant `:uuid`).
- **Fait — `src/structure/transfer-targets.service.spec.ts`** (3 tests) : la cascade descend une
  branche **sans lien** avec l'appelant, et refuse de descendre sous le district.
- **Observation hors périmètre :** la table `responsibilities` ne contient **aucune responsabilité
  de niveau `CENTRE_REGIONAL`** (NATIONAL 10, REGION 5, CENTRE 5, CHAPITRE 4, DISTRICT 4, GROUPE 1,
  SOUS_GROUPE 1). Personne ne peut donc être cadré à ce palier. Sans effet sur ce correctif, mais à
  arbitrer — module `responsabilités`.
- **TODO :** recette de bout en bout avec un compte `RESPONSABLE` réel (le test par jeton forgé
  n'a pas pu être joué) ; correctif web associé dans `web/docs/JOURNAL.md`, même date ;
  `graphify update .` toujours en attente.
